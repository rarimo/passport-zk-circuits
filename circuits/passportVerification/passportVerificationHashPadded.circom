...
include "../hasher/sha2/sha256/sha256HashBits.circom";
include "circomlib/circuits/poseidon.circom";
include "../rsa/rsa.circom";
include "../sha256/sha256NoPadding.circom";
include "../merkleTree/SMTVerifier.circom";
// include "../x509Verification/X509Verifier.circom";  // dead code, stale API
...
template PassportVerificationHashPadded(BLOCK_SIZE, NUMBER_OF_BLOCKS, E_BITS, HASH_BLOCKS_NUMBER, TREE_DEPTH) {
    ...
    component encapsulatedContentHasher4Blocks = Sha256NoPadding(4);
    ...
    component encapsulatedContentHasher5Blocks = Sha256NoPadding(5);
    for (var i = 0; i < HASH_BLOCK_SIZE * 5; i++) {
    ...
    // Hashing the FULL modulus with SHA256; leaf = top 248 bits
    var KEY_BITS = NUMBER_OF_BLOCKS * BLOCK_SIZE;

    component num2bitsPk[NUMBER_OF_BLOCKS];
    signal modBits[KEY_BITS];
    for (var i = 0; i < NUMBER_OF_BLOCKS; i++){
        num2bitsPk[i] = Num2Bits(BLOCK_SIZE);
        num2bitsPk[i].in <== modulus[i];
        for (var b = 0; b < BLOCK_SIZE; b++){
            // big-endian (MSB-first) bit stream for SHA
            modBits[KEY_BITS - 1 - (i * BLOCK_SIZE + b)] <== num2bitsPk[i].out[b];
        }
    }

    component modulusHasher = Sha256HashBits(KEY_BITS);
    modulusHasher.in <== modBits;

    component pubkeyLeaf = Bits2Num(248);
    for (var k = 0; k < 248; k++){
        pubkeyLeaf.in[k] <== modulusHasher.out[247 - k];
    }
    ...
    smtVerifier.leaf <== pubkeyLeaf.out;
    smtVerifier.key <== pubkeyLeaf.out;
