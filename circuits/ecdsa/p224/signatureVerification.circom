...
template verifyP224(CHUNK_SIZE, CHUNK_NUMBER, ALGO) {
    signal input pubkey[2 * 224];
    signal input signature[2 * 224];
    ...
    for (var i = 0; i < 2; i++){
        for (var j = 0; j < 224; j++){
            pubkeyBits[i][j + 0] <== pubkey[i * 224 + j];
            signatureBits[i][j + 0] <== signature[i * 224 + j];
        }
    }
    ...
    component getOrder = GetP224Order(CHUNK_SIZE,CHUNK_NUMBER);
    ...
    component scalarMult1 = P224GeneratorMultiplication(CHUNK_SIZE,CHUNK_NUMBER);
    component scalarMult2 = P224PipingerMult(CHUNK_SIZE,CHUNK_NUMBER,4);
    ...
    component sumPoints = P224AddUnequal(CHUNK_SIZE,CHUNK_NUMBER);
