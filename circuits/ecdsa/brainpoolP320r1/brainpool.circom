...
template Brainpool320ScalarMult320(CHUNK_SIZE, CHUNK_NUMBER){
    ...
    for (var i = CHUNK_NUMBER - 1; i >= 0; i--) {
        for (var j = CHUNK_SIZE - 1; j >= 0; j--) {
            ...
            if (i < CHUNK_NUMBER - 1 || j < CHUNK_SIZE - 1) {
                adders[CHUNK_SIZE * i + j] = Brainpool320AddUnequal320(CHUNK_SIZE, CHUNK_NUMBER);
                doublers[CHUNK_SIZE * i + j] = Brainpool320Double320(CHUNK_SIZE, CHUNK_NUMBER);
                for (var idx = 0; idx < CHUNK_NUMBER; idx++) {
...
template Brainpool320AddUnequal320(CHUNK_SIZE, CHUNK_NUMBER){
...
template Brainpool320Double320(CHUNK_SIZE, CHUNK_NUMBER){
...
template GetBrainpool320320Order(CHUNK_SIZE, CHUNK_NUMBER){
...
template Brainpool320320GeneratorMultiplication(CHUNK_SIZE, CHUNK_NUMBER){
    var STRIDE = 8;
    signal input scalar[CHUNK_NUMBER];
    signal output out[2][CHUNK_NUMBER];
    ...
    for (var i = 1; i < NUM_STRIDES; i++) {
        adders[i - 1] = Brainpool320AddUnequal320(CHUNK_SIZE, CHUNK_NUMBER);
        for (var idx = 0; idx < CHUNK_NUMBER; idx++) {
            for (var l = 0; l < 2; l++) {
...
template Brainpool320320PrecomputePipinger(CHUNK_SIZE, CHUNK_NUMBER, WINDOW_SIZE){
    signal input in[2][CHUNK_NUMBER];
    ...
    for (var i = 2; i < PRECOMPUTE_NUMBER; i++){
        if (i % 2 == 0){
            doublers[i\2 - 1]     = Brainpool320Double320(CHUNK_SIZE, CHUNK_NUMBER);
            doublers[i\2 - 1].in  <== out[i\2];
            doublers[i\2 - 1].out ==> out[i];
        }
        else
        {
            adders[i\2 - 1]          = Brainpool320AddUnequal320(CHUNK_SIZE, CHUNK_NUMBER);
...
template Brainpool320PipingerMult320(CHUNK_SIZE, CHUNK_NUMBER, WINDOW_SIZE){
    ...
    component precompute = Brainpool320320PrecomputePipinger(CHUNK_SIZE, CHUNK_NUMBER, WINDOW_SIZE);
    precompute.in  <== point;
    ...
    for (var i = 0; i < CHUNK_NUMBER*CHUNK_SIZE; i += WINDOW_SIZE){
        adders[i\WINDOW_SIZE] = Brainpool320AddUnequal320(CHUNK_SIZE, CHUNK_NUMBER);
        ...
        if (i != 0){
            for (var j = 0; j < WINDOW_SIZE; j++){
                doublers[i + j - WINDOW_SIZE] = Brainpool320Double320(CHUNK_SIZE, CHUNK_NUMBER);
