...
template Fp12CyclotomicCompress(CHUNK_SIZE, CHUNK_NUMBER) {
    ...
    for(var eps=0; eps<2; eps++)for(var j=0; j<CHUNK_NUMBER; j++){
...
template Fp12CyclotomicDecompress(CHUNK_SIZE, CHUNK_NUMBER, p) {
    ...
    for(var i=0; i<CHUNK_NUMBER; i++)for(var eps=0; eps<2; eps++){
    ...
    for(var i=0; i<2; i++)for(var j=0; j<2*CHUNK_NUMBER-1; j++)
...
template Fp12CyclotomicSquare(CHUNK_SIZE, CHUNK_NUMBER, p) {
    ...
    for(var i=0; i<4; i++)for(var eps=0; eps<2; eps++)for(var j=0; j<CHUNK_NUMBER; j++){
    ...
    for(var i=0; i<4; i++){
        ...
        for(var eps=0; eps<2; eps++)for(var j=0; j<2*CHUNK_NUMBER-1; j++)
...
template Fp12CyclotomicExp(CHUNK_SIZE, CHUNK_NUMBER, e, p) {
    ...
    for(var i=0; i<6; i++)for(var eps=0; eps<2; eps++)for(var j=0; j<CHUNK_NUMBER; j++)
    ...
    for(var i=0; i<BITLENGTH; i++){
        ...
        if( i > 0 ){ // pow2[0] is never defined since there is no squaring involved
            ...
            if( i == 1 ){
                for(var id=0; id<4; id++)for(var eps=0; eps<2; eps++)for(var j=0; j<CHUNK_NUMBER; j++)
