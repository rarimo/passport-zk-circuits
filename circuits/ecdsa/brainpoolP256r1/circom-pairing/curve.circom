...
template EllipticCurveAddUnequal(n, k, p) { 
    ...
    var dy[150] = long_sub_mod(n, k, b[1], a[1], p);
    var dx[150] = long_sub_mod(n, k, b[0], a[0], p); 
    var dx_inv[150] = mod_inv(n, k, dx, p);
    var lambda[150] = prod_mod(n, k, dy, dx_inv, p);
    var lambda_sq[150] = prod_mod(n, k, lambda, lambda, p);
    // out[0] = x_3 = lamb^2 - a[0] - b[0] % p
    // out[1] = y_3 = lamb * (a[0] - x_3) - a[1] % p
    var x3[150] = long_sub_mod(n, k, long_sub_mod(n, k, lambda_sq, a[0], p), b[0], p);
    var y3[150] = long_sub_mod(n, k, prod_mod(n, k, lambda, long_sub_mod(n, k, a[0], x3, p), p), a[1], p);
...
template EllipticCurveDouble(n, k, a, b, p) {
    ...
    // precompute lambda 
    var lamb_num[150] = long_add_mod(n, k, a, prod_mod(n, k, long_3, prod_mod(n, k, in[0], in[0], p), p), p);
    var lamb_denom[150] = long_add_mod(n, k, in[1], in[1], p);
    var lamb[150] = prod_mod(n, k, lamb_num, mod_inv(n, k, lamb_denom, p), p);

    // precompute x_3, y_3
    var x3[150] = long_sub_mod(n, k, prod_mod(n, k, lamb, lamb, p), long_add_mod(n, k, in[0], in[0], p), p);
    var y3[150] = long_sub_mod(n, k, prod_mod(n, k, lamb, long_sub_mod(n, k, in[0], x3, p), p), in[1], p);
