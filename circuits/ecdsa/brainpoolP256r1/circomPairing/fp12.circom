...
template SignedFp12Fp2MultiplyNoCarry(CHUNK_SIZE, CHUNK_NUMBER, M_OUT){
    ...
    for(var i=0; i<6; i++)for(var idx=0; idx<2*CHUNK_NUMBER-1; idx++) {
...
template SignedFp12MultiplyNoCarry(CHUNK_SIZE, CHUNK_NUMBER, M_OUT){
    ...
    for(var i=0; i<L; i++)for(var j=0; j<2; j++)for(var idx=0; idx<CHUNK_NUMBER; idx++){
        mult.a[i][j][idx] <== a[i][j][idx];
        mult.b[i][j][idx] <== b[i][j][idx];
    }
    for(var i=0; i<L; i++)for(var j=0; j<2; j++)for(var idx=0; idx<2*CHUNK_NUMBER-1; idx++)
...
template Fp12Square(CHUNK_SIZE, CHUNK_NUMBER, P) {
    ...
    for(var i=0; i<6; i++)for(var j=0; j<CHUNK_NUMBER; j++){
...
template Fp12Invert(CHUNK_SIZE, CHUNK_NUMBER, P){
    ...
    for(var i=0; i<6; i++) for(var j=0; j<2; j++) for(var M=0; M<CHUNK_NUMBER; M++) {
...
template Fp12Exp(CHUNK_SIZE, CHUNK_NUMBER, e, P) {
    ...
    for(var i=0; i<BITLENGTH; i++){
        ...
        if( i > 0 ){ // pow2[0] is never defined since there is no squaring involved
            ...
            for(var j=0; j<CHUNK_NUMBER; j++) pow2[i].P[j] <== P[j];
            if( i == 1 ){
                for(var id=0; id<6; id++)for(var eps=0; eps<2; eps++)for(var j=0; j<CHUNK_NUMBER; j++)
