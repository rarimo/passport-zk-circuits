...
template InternalRound(i) {
    ...
    sigma.in <== in[0] + ROUND_CONSTANTS[i];
    
    out[0] <== 2 * sigma.out + in[1] + in[2];
    out[1] <== sigma.out + 2 * in[1] + in[2];
    out[2] <== sigma.out + in[1] + 3 * in[2];
...
template ExternalRound(i) {
    ...
    for (var j = 0; j < 3; j++) {
        sigma[j] = Sigma();
        sigma[j].in <== in[j] + ROUND_CONSTANTS[i][j];
    }
    
    out[0] <== 2 * sigma[0].out + sigma[1].out + sigma[2].out;
    out[1] <== sigma[0].out + 2 * sigma[1].out + sigma[2].out;
    out[2] <== sigma[0].out + sigma[1].out + 2 * sigma[2].out;
}
...
template Permutation() {
    ...
    component ext[8];
    for (var k = 0; k < 8; k++) {
        ext[k] = ExternalRound(k);
    }
    
    component int[56];
    for (var k = 0; k < 56; k++) {
        int[k] = InternalRound(k);
    }
    ...
    for (var k = 0; k < 56; k++) {
        for (var j = 0; j < 3; j++) {
            int[k].in[j] <== aux[k + 4][j];
        }
        for (var j = 0; j < 3; j++) {
            int[k].out[j] ==> aux[k + 5][j];
        }
    }
    ...
    for (var k = 0; k < 4; k++) {
        for (var j = 0; j < 3; j++) {
            ext[k + 4].in[j] <== aux[k + 60][j];
        }
        for (var j = 0; j < 3; j++) {
            ext[k + 4].out[j] ==> aux[k + 61][j];
        }
    }
