...
template QueryIdentity(idTreeDepth) {
    ...
    signal output personalNumberHash;  // PoseidonHash(UTF-8 encoded)
    ...
    signal input dg1[760];            // 744 bits | DG1 in binary
    ...
    component documentNumberHasher = PoseidonHash(1);
    documentNumberHasher.in[0] <== dg1DataExtractor.documentNumber;

    // Personal number hasher
    component personalNumberHasher = PoseidonHash(1);
    personalNumberHasher.in[0] <== dg1DataExtractor.personalNumber;
    ...
    component skIdentityHasher = PoseidonHash(1);
    skIdentityHasher.in[0] <== skIdentity;
    // nullifier => Poseidon3(sk_i, Poseidon1(sk_i), eventID)
    component nulliferHasher = PoseidonHash(3);
    nulliferHasher.in[0] <== skIdentity;
    nulliferHasher.in[1] <== skIdentityHasher.out;
    nulliferHasher.in[2] <== eventID;
    ...
    // Retrieve DGCommit: DG1 hash 760 bits => 4 * 190
    component dg1Chunking[4];
    component dg1Hasher = PoseidonHash(5);
    var DG1_COMMITMENT_CHUNK_SIZE = 190;
    for (var i = 0; i < 4; i++) {
