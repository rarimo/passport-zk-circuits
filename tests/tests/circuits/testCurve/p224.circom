...
include "../../../..circuits/ecdsa/brainpoolP256r1/circomPairing/curve.circom";
include "p224Func.circom";
include "p224Pows.circom";
...
template P224AddUnequal(CHUNK_SIZE, CHUNK_NUMBER){
    signal input point1[2][CHUNK_NUMBER];
    signal input point2[2][CHUNK_NUMBER];
    signal output out[2][CHUNK_NUMBER];


    var PARAMS[3][CHUNK_NUMBER] = get_p224_params(CHUNK_SIZE,CHUNK_NUMBER);
...
template P224Double(CHUNK_SIZE, CHUNK_NUMBER){
...
template P224ScalarMult(CHUNK_SIZE, CHUNK_NUMBER){
    ...
    for (var i = CHUNK_NUMBER - 1; i >= 0; i--) {
        for (var j = CHUNK_SIZE - 1; j >= 0; j--) {
            ...
            if (i < CHUNK_NUMBER - 1 || j < CHUNK_SIZE - 1) {
                adders[CHUNK_SIZE * i + j] = P224AddUnequal(CHUNK_SIZE, CHUNK_NUMBER);
                doublers[CHUNK_SIZE * i + j] = P224Double(CHUNK_SIZE, CHUNK_NUMBER);
                for (var idx = 0; idx < CHUNK_NUMBER; idx++) {
...
template GetP224Order(CHUNK_SIZE, CHUNK_NUMBER){
    assert((CHUNK_SIZE == 32) && (CHUNK_NUMBER == 7));
    signal output order[7];
    order[0] <== 1549543997;
    order[1] <== 333261125;
    order[2] <== 3770216510;
    order[3] <== 4294907554;
...
template GetP224Generator(CHUNK_SIZE,CHUNK_NUMBER){
    assert((CHUNK_SIZE == 32) && (CHUNK_NUMBER == 7));
    signal output generator[2][7];


    generator[0][0] <== 291249441;
    generator[0][1] <== 875725014;
    generator[0][2] <== 1455558946;
    generator[0][3] <== 1241760211;
    generator[0][4] <== 840143033;
    generator[0][5] <== 1807007615;
    generator[0][6] <== 3071151293;
    generator[1][0] <== 2231402036;
    generator[1][1] <== 1154843033;
    generator[1][2] <== 1510426468;
    generator[1][3] <== 3443750304;
    generator[1][4] <== 1277353958;
    generator[1][5] <== 3052872699;
    generator[1][6] <== 3174523784;
}


template P224GeneratorMultiplication(CHUNK_SIZE,CHUNK_NUMBER){
    var STRIDE = 8;
    signal input scalar[CHUNK_NUMBER];
    signal output out[2][CHUNK_NUMBER];
    ...
    var dummyHolder[2][CHUNK_NUMBER] = get_p224_dummy_point(CHUNK_SIZE, CHUNK_NUMBER);
    ...
    for (var i = 1; i < NUM_STRIDES; i++) {
        adders[i - 1] = P224AddUnequal(CHUNK_SIZE, CHUNK_NUMBER);
        for (var idx = 0; idx < CHUNK_NUMBER; idx++) {
            for (var l = 0; l < 2; l++) {
...
template P224PrecomputePipinger(CHUNK_SIZE, CHUNK_NUMBER, WINDOW_SIZE){
    signal input in[2][CHUNK_NUMBER];
    ...
    for (var i = 2; i < PRECOMPUTE_NUMBER; i++){
        if (i % 2 == 0){
            doublers[i\2 - 1]     = P224Double(CHUNK_SIZE, CHUNK_NUMBER);
            doublers[i\2 - 1].in  <== out[i\2];
            doublers[i\2 - 1].out ==> out[i];
        }
        else
        {
            adders[i\2 - 1]          = P224AddUnequal(CHUNK_SIZE, CHUNK_NUMBER);
...
template P224PipingerMult(CHUNK_SIZE, CHUNK_NUMBER, WINDOW_SIZE){
    ...
    component precompute = P224PrecomputePipinger(CHUNK_SIZE, CHUNK_NUMBER, WINDOW_SIZE);
    precompute.in  <== point;
    precompute.out ==> precomputed;


    var DOUBLERS_NUMBER = 224 - WINDOW_SIZE;
    var ADDERS_NUMBER   = 224 \ WINDOW_SIZE;
    ...
    component g = GetP224Generator(CHUNK_SIZE, CHUNK_NUMBER);
    signal gen[2][CHUNK_NUMBER];
    gen <== g.generator;


    signal scalarBits[224];
    ...
    for (var i = 0; i < CHUNK_NUMBER; i++){
        ...
        if (i != CHUNK_NUMBER - 1){
            for (var j = 0; j < CHUNK_SIZE; j++){
                scalarBits[224 - CHUNK_SIZE * (i + 1) + j] <== num2Bits[i].out[CHUNK_SIZE - 1 - j];
            }
        } else {
            for (var j = 0; j < CHUNK_SIZE - (CHUNK_SIZE*CHUNK_NUMBER - 224); j++){
                scalarBits[j] <== num2Bits[i].out[CHUNK_SIZE - 1 - (j + (CHUNK_SIZE * CHUNK_NUMBER - 224))];
            }
        }
    }
    ...
    for (var i = 0; i < 224; i += WINDOW_SIZE){
        adders[i\WINDOW_SIZE] = P224AddUnequal(CHUNK_SIZE, CHUNK_NUMBER);
        ...
        if (i != 0){
            for (var j = 0; j < WINDOW_SIZE; j++){
                doublers[i + j - WINDOW_SIZE] = P224Double(CHUNK_SIZE, CHUNK_NUMBER);
