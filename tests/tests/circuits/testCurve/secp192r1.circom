...
template Secp192r1AddUnequal(CHUNK_SIZE, CHUNK_NUMBER){
    signal input point1[2][CHUNK_NUMBER];
    signal input point2[2][CHUNK_NUMBER];
    signal output out[2][CHUNK_NUMBER];


    var PARAMS[3][CHUNK_NUMBER] = get_secp192r1_params(CHUNK_SIZE,CHUNK_NUMBER);
...
template Secp192r1Double(CHUNK_SIZE, CHUNK_NUMBER){
...
template Secp192r1ScalarMult(CHUNK_SIZE, CHUNK_NUMBER){
    ...
    for (var i = CHUNK_NUMBER - 1; i >= 0; i--) {
        for (var j = CHUNK_SIZE - 1; j >= 0; j--) {
            ...
            if (i < CHUNK_NUMBER - 1 || j < CHUNK_SIZE - 1) {
                adders[CHUNK_SIZE * i + j] = Secp192r1AddUnequal(CHUNK_SIZE, CHUNK_NUMBER);
                doublers[CHUNK_SIZE * i + j] = Secp192r1Double(CHUNK_SIZE, CHUNK_NUMBER);
                for (var idx = 0; idx < CHUNK_NUMBER; idx++) {
...
template GetSecp192r1Order(CHUNK_SIZE, CHUNK_NUMBER){
...
template GetSecp192r1Generator(CHUNK_SIZE,CHUNK_NUMBER){
    assert((CHUNK_SIZE == 32) && (CHUNK_NUMBER == 6));
    signal output generator[2][6];


    generator[0][0] <== 2197753874;
    generator[0][1] <== 4110355197;
    generator[0][2] <== 1134659584;
    generator[0][3] <== 2092900587;
    generator[0][4] <== 2955972854;
    generator[0][5] <== 411936782;
    generator[1][0] <== 511264785;
    generator[1][1] <== 1945728929;
    generator[1][2] <== 1797574101;
    generator[1][3] <== 1661997549;
    generator[1][4] <== 4291353208;
    generator[1][5] <== 119090069;
}


template Secp192r1GeneratorMultiplication(CHUNK_SIZE,CHUNK_NUMBER){
    var STRIDE = 8;
    signal input scalar[CHUNK_NUMBER];
    signal output out[2][CHUNK_NUMBER];
    ...
    for (var i = 1; i < NUM_STRIDES; i++) {
        adders[i - 1] = Secp192r1AddUnequal(CHUNK_SIZE, CHUNK_NUMBER);
        for (var idx = 0; idx < CHUNK_NUMBER; idx++) {
            for (var l = 0; l < 2; l++) {
...
template Secp192r1PrecomputePipinger(CHUNK_SIZE, CHUNK_NUMBER, WINDOW_SIZE){
    signal input in[2][CHUNK_NUMBER];
    ...
    for (var i = 2; i < PRECOMPUTE_NUMBER; i++){
        if (i % 2 == 0){
            doublers[i\2 - 1]     = Secp192r1Double(CHUNK_SIZE, CHUNK_NUMBER);
            doublers[i\2 - 1].in  <== out[i\2];
            doublers[i\2 - 1].out ==> out[i];
        }
        else
        {
            adders[i\2 - 1]          = Secp192r1AddUnequal(CHUNK_SIZE, CHUNK_NUMBER);
...
template Secp192r1PipingerMult(CHUNK_SIZE, CHUNK_NUMBER, WINDOW_SIZE){
    ...
    component precompute = Secp192r1PrecomputePipinger(CHUNK_SIZE, CHUNK_NUMBER, WINDOW_SIZE);
    precompute.in  <== point;
    precompute.out ==> precomputed;


    var DOUBLERS_NUMBER = 192 - WINDOW_SIZE;
    var ADDERS_NUMBER   = 192 \ WINDOW_SIZE;
    ...
    component g = GetSecp192r1Generator(CHUNK_SIZE, CHUNK_NUMBER);
    signal gen[2][CHUNK_NUMBER];
    gen <== g.generator;


    signal scalarBits[192];
    ...
    for (var i = 0; i < CHUNK_NUMBER; i++){
        ...
        if (i != CHUNK_NUMBER - 1){
            for (var j = 0; j < CHUNK_SIZE; j++){
                scalarBits[192 - CHUNK_SIZE * (i + 1) + j] <== num2Bits[i].out[CHUNK_SIZE - 1 - j];
            }
        } else {
            for (var j = 0; j < CHUNK_SIZE - (CHUNK_SIZE*CHUNK_NUMBER - 192); j++){
                scalarBits[j] <== num2Bits[i].out[CHUNK_SIZE - 1 - (j + (CHUNK_SIZE * CHUNK_NUMBER - 192))];
            }
        }
    }
    ...
    for (var i = 0; i < 192; i += WINDOW_SIZE){
        adders[i\WINDOW_SIZE] = Secp192r1AddUnequal(CHUNK_SIZE, CHUNK_NUMBER);
        ...
        if (i != 0){
            for (var j = 0; j < WINDOW_SIZE; j++){
                doublers[i + j - WINDOW_SIZE] = Secp192r1Double(CHUNK_SIZE, CHUNK_NUMBER);
